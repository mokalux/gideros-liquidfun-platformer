...
# contour = tracer + cleaner + shaper library for Gideros
...
function Contour.clean(con, dist)
	...
	if k > 2 and slopesNearColinear(out[k-2], out[k-1], out[1], distSqrd) then
		k = k - 1
	end    
	if k <= l then
...
function Contour.apply(body, shapes, fixdef, filter) -- filter new 2019-10-27
	...
	for i, shape in ipairs(shapes) do
		...
		local fixture = body:createFixture(fixdef)
		if filter ~= nil then
